...
ui64_t history = arg_history;
  ...
  if ((ncaptx_you == 0) or (ncaptx_you == 0xF)) return(0);
  ...
  while(TRUE)
  {
    ...
    if ((ncaptx_me > 0) and (ncaptx_you == 0)) ++result;
...
local int quiescence(search_t *object, int arg_my_alpha, int arg_my_beta, 
  int arg_node_type, moves_list_t *arg_moves_list,
  int arg_all_moves, int arg_ncapture_extensions, ui64_t arg_history,
  ...
  *arg_best_depth = INVALID;
      ...
      (arg_my_alpha >= (SCORE_LOST + NODE_MAX)))
  {
    ...
    else if (options.quiescence_extension_search_window > 0)
    {
      ...
      temp_score = quiescence(object, reduced_alpha, reduced_beta,
        MINIMAL_WINDOW_BIT, arg_moves_list,
        TRUE, arg_ncapture_extensions, arg_history,
        temp_pv, &temp_depth);
  ...
  if ((arg_ncapture_extensions > 0) and (!arg_all_moves) and
      (nextend == 0))
  { 
  ...
  for (int imove = 0; imove < arg_moves_list->ML_nmoves; imove++)
  {
    ...
    if (IS_MINIMAL_WINDOW(arg_node_type))
    {
      ...
      temp_score = -quiescence(object, -arg_my_beta, -temp_alpha,
        arg_node_type, NULL,
        FALSE, arg_ncapture_extensions, arg_history,
    ...
    else
    {
      ...
      else
      {
        ...
        temp_score = -quiescence(object, -temp_beta, -temp_alpha,
          MINIMAL_WINDOW_BIT, NULL,
          FALSE, arg_ncapture_extensions, arg_history,
          temp_pv + 1, &temp_depth);
...
local int alpha_beta(search_t *object,
  ...
  int arg_node_type, int arg_reduction_depth_root,
  int arg_my_tweaks, ui64_t arg_history,
  pv_t *arg_best_pv, int *arg_best_depth)
{
  PUSH_NAME(__FUNC__)
  ...
  *arg_best_depth = INVALID;
  ...
  if (options.alpha_beta_cache_size > 0)
  {
    alpha_beta_cache_hit = probe_alpha_beta_cache(object, arg_node_type, FALSE,
      alpha_beta_cache[my_colour], &alpha_beta_cache_slot);
  ...
  arg_history = (arg_history << 4) | (moves_list.ML_ncaptx & 0xF) |
                0xF000000000000000ULL;
    ...
    for (int imove = 0; imove < moves_list.ML_nmoves; imove++)
    {
      ...
      if (IS_MINIMAL_WINDOW(arg_node_type))
      {
        ...
        temp_score = -alpha_beta(object,
          -arg_my_beta, -temp_alpha, arg_my_depth,
          arg_node_type, arg_reduction_depth_root,
          your_tweaks, arg_history,
          temp_pv + 1, &temp_depth);
      ...
      else
      {
        ...
        else
        {
          ...
          temp_score = -alpha_beta(object,
            -temp_beta, -temp_alpha, arg_my_depth,
            MINIMAL_WINDOW_BIT, arg_reduction_depth_root,
            your_tweaks, arg_history,
            temp_pv + 1, &temp_depth);
  ...
  if (your_depth < 0)
  {
    best_score = quiescence(object, arg_my_alpha, arg_my_beta,
      arg_node_type, &moves_list,
      FALSE, options.quiescence_capture_extensions, arg_history,
      arg_best_pv, arg_best_depth);
  ...
  if (IS_MINIMAL_WINDOW(arg_node_type))
  {
    ...
    if (allow_reductions and 
        (options.reduction_combination_length > 0) and
        (return_combination_length(arg_history) >=
    ...
    for (int ipass = 1; ipass <= 2; ++ipass)
    {
      for (int imove = 0; imove < moves_list.ML_nmoves; imove++)
      {
        ...
        if (ipass == 1)
        {
          for (jmove = 0; jmove < moves_list.ML_nmoves; jmove++)
          {
            if ((stage[jmove] == STAGE_SEARCH_MOVE_AT_REDUCED_DEPTH) or
                (stage[jmove] == STAGE_SEARCH_MOVE_AT_FULL_DEPTH)) break;
          }
          ...
          for (int kmove = jmove + 1; kmove < moves_list.ML_nmoves; kmove++)
          {
            if ((stage[kmove] != STAGE_SEARCH_MOVE_AT_REDUCED_DEPTH) and
                (stage[kmove] != STAGE_SEARCH_MOVE_AT_FULL_DEPTH)) continue;
        ...
        else
        {
          ...
          for (jmove = 0; jmove < moves_list.ML_nmoves; jmove++)
            if (stage[jmove] == STAGE_RESEARCH_MOVE_AT_FULL_DEPTH) break;

          if (jmove >= moves_list.ML_nmoves) break;

          for (int kmove = 1; kmove < moves_list.ML_nmoves; kmove++)
          {
            if (stage[kmove] != stage[jmove]) continue;

            if (reduced_score[kmove] > reduced_score[jmove]) jmove = kmove;
          }

          if (reduced_score[jmove] <
              (best_score - options.reduction_research_margin)) break;
        ...
        if (stage[jmove] == STAGE_SEARCH_MOVE_AT_REDUCED_DEPTH)
        {
          for (int iprobe = options.reduction_probes; iprobe >= 0; --iprobe)
          {
            ...
            temp_score = -alpha_beta(object,
              ...
              arg_node_type, arg_reduction_depth_root,
              your_tweaks | TWEAK_PREVIOUS_MOVE_REDUCED_BIT, arg_history,
              temp_pv + 1, &temp_depth);
            ...
            if (temp_score <= reduced_alpha) break;
          }
  ...
  else //IS_MINIMAL_WINDOW(node_type) NEW
  {
        ...
        (arg_my_alpha >= (SCORE_LOST + NODE_MAX)))
    {
      ...
      else if (options.pv_extension_search_window > 0)
      {
        ...
        int temp_score = alpha_beta(object,
          reduced_alpha, reduced_beta, reduced_depth,
          MINIMAL_WINDOW_BIT, arg_reduction_depth_root,
          arg_my_tweaks | TWEAK_PREVIOUS_SEARCH_EXTENDED_BIT, arg_history,
          temp_pv, &temp_depth);
...
void do_pv(search_t *object, pv_t *arg_best_pv, int arg_pv_score, bstring arg_bpv_string)
{
  colour_enum my_colour;
...
local void print_pv(search_t *object, int arg_my_depth,
...
void do_search(search_t *object, moves_list_t *arg_moves_list,
  ...
  my_random_t *arg_shuffle)
{
  ...
  { 
    ...
    object->S_root_score = -alpha_beta(object,
      SCORE_MINUS_INFINITY, SCORE_PLUS_INFINITY, 1,
      PV_BIT, options.reduction_depth_root,
      TWEAK_PREVIOUS_MOVE_NULL_BIT, 0xFULL,
      temp_pv + 1, &temp_depth);
      ...
      for (int imove = 0; imove < arg_moves_list->ML_nmoves; imove++)
      {
        ...
        while(TRUE)
        {
          ...
          move_score = -alpha_beta(object,
            ...
            node_type, options.reduction_depth_root,
            your_tweaks, 0xFULL,
            temp_pv + 1, &temp_depth);
          ...
          if (move_score <= my_alpha)
          {
            ...
            if ((nbest_moves == 0) and
                (best_score_margin > 0) and
            ...
            print_pv(object, my_depth, 0, jmove, arg_moves_list, move_score,
                     temp_pv, "<=");
  
            if (iteration > 0) window_alpha *= 2;
          }
        ...
        if ((nbest_moves == 0) or (move_score > object->S_best_score))
        {
          ...
          print_pv(object, my_depth, imove, jmove, arg_moves_list, move_score,
                   object->S_best_pv, "===");
...
void init_search(void)
{
  ...
  if (options.alpha_beta_cache_size > 0)
  {
    ...
    MY_MALLOC_BY_TYPE(alpha_beta_cache[WHITE_ENUM], alpha_beta_cache_entry_t,
              nalpha_beta_pv_cache_entries + nalpha_beta_cache_entries)
  
    MY_MALLOC_BY_TYPE(alpha_beta_cache[BLACK_ENUM], alpha_beta_cache_entry_t,
              nalpha_beta_pv_cache_entries + nalpha_beta_cache_entries)
