...
local endgame_t
    endgames[NENDGAME_MAX][NENDGAME_MAX][NENDGAME_MAX][NENDGAME_MAX];
...
local void convert2wdl(void)
{
  ...
  cJSON_ArrayForEach(egtb, egtbs)
  {
    ...
    if ((npages * nblock_entries) < (index_max + 1))
      npages++;
    ...
    for (i64_t iwdl = 0; iwdl < nwdl; iwdl++)
      pwdl_t[iwdl] = 0;
...
void init_endgame(void)
{
  ...
  my_mpi_barrier_v3("convert2wdl",
                    my_mpi_globals.MMG_comm_global,
                    my_mpi_globals.MMG_semkey_init_endgame_barrier,
                    TRUE);
    ...
    for (nwm = 0; nwm < NENDGAME_MAX; nwm++)
    {
      for (nbk = 0; nbk < NENDGAME_MAX; nbk++)
      {
        for (nbm = 0; nbm < NENDGAME_MAX; nbm++)
        {
          ...
          snprintf(with_endgame->endgame_name,
                   MY_LINE_MAX,
                   "%dwX-%dwO-%dbX-%dbO",
                   nwk,
                   nwm,
          ...
          HARDBUG(fread(&index_max, sizeof(i64_t), 1, with_endgame->fendgame) !=
                  1)
          ...
          cJSON_ArrayForEach(egtb, egtbs)
          {
            ...
            my_mpi_allocate_shared(my_mpi_globals.MMG_comm_global,
                                   sizeof(wdl_t),
                                   nwdl,
                                   &(with_endgame->pendgame),
                                   &(with_endgame->wendgame));
            ...
            if (my_mpi_globals.MMG_id_global <= 0)
            {
              ...
              HARDBUG(fread(with_endgame->pendgame,
                            sizeof(i8_t),
                            wdl_size,
    ...
    for (nwm = 0; nwm < NENDGAME_MAX; nwm++)
    {
      for (nbk = 0; nbk < NENDGAME_MAX; nbk++)
      {
        for (nbm = 0; nbm < NENDGAME_MAX; nbm++)
        {
          ...
          if (with_endgame->endgame_status == ENDGAME_ALPHA_BETA_WDL)
          {
...
void update_endgame_index(int arg_nlist,
                          int arg_list[NENDGAME_MAX],
                          i64_t *endgame_index)
{
  if (arg_nlist == 0)
    return;
...
#ifdef ENABLE_RECOMPRESSION
local void decode(i8_t *arg_flag,
                  i16_t *arg_shift,
                  ui8_t arg_code[ARRAY_PAGE_SIZE],
...
int read_endgame(search_t *object, colour_enum arg_colour2move, int *arg_wdl)
{
  ...
  if ((arg_wdl != NULL) and (with_endgame->endgame_status == ENDGAME_ROOT_ONLY))
  {
    *arg_wdl = FALSE;
  ...
  else
  {
...
#ifndef ZSTD
    i64_t seek = sizeof(i64_t) +
                 ipage * (sizeof(i64_t) + sizeof(i16_t) + sizeof(ui64_t));
#else
    i64_t seek = sizeof(i8_t) + sizeof(i16_t) + sizeof(i64_t) +
                 ipage * (sizeof(i64_t) + sizeof(i16_t) + sizeof(ui32_t));
#endif
...
#ifndef ZSTD
      memcpy(source,
             (i8_t *)with_endgame->pendgame + offset,
...
#ifndef ZSTD
      HARDBUG(fread(source,
                    sizeof(COMPRESS_BYTE_T),
                    sourceLen,
                    with_endgame->fendgame) != sourceLen)
#else
      HARDBUG(fread(source, sizeof(i8_t), sourceLen, with_endgame->fendgame) !=
              sourceLen)
#endif
    ...
    else if (source[0] == ALL_I16)
    {
      ...
      destLen = ZSTD_decompress(page, destLen, source + 1, sourceLen);
...
void recompress_endgame(char *arg_name, int arg_level, int arg_nblock)
{
  ...
  if ((npage_old * ARRAY_PAGE_SIZE) <=
      (index_max_old + 1) * sizeof(entry_i16_t))
    npage_old++;
  ...
  for (i64_t ipage_old = 0; ipage_old < npage_old; ipage_old++)
  {
    ...
    HARDBUG(fread(source_old, sizeof(COMPRESS_BYTE_T), sourceLen_old, fold) !=
            sourceLen_old)
    ...
    if (source_old[0] != ENCODE_DRAW)
    {
      ...
      if ((err = UNCOMPRESS(dest_old,
                            &destLen_old,
                            source_old + 1,
                            sourceLen_old)) != COMPRESS_OK)
      {
        PRINTF("err=%d\n", err);
        PRINTF("name=%s\n", arg_name);
        FATAL("uncompress error", EXIT_FAILURE)
    ...
    for (int iblock = 0; iblock < arg_nblock; iblock++)
    {
      ...
      for (int ientry = 0; ientry < nblock_entries; ientry++)
      {
        ...
        if (white_mate != INVALID)
          all_draw = FALSE;
        if (black_mate != INVALID)
          all_draw = FALSE;
      ...
      else if (all_i8)
      {
        ...
        destLen_new = ZSTD_compress(dest_new + 1,
                                    destLen_new - 1,
                                    source_new,
                                    sourceLen_new,
                                    arg_level);
        ...
        sourceLen_newer = ZSTD_decompress(source_newer,
                                          sourceLen_newer,
                                          dest_new + 1,
                                          destLen_new);
      ...
      else
      {
        ...
        {
          crc32 =
              HW_CRC32_U16(crc32, page_i16[jentry + ientry].entry_white_mate);
          crc32 =
              HW_CRC32_U16(crc32, page_i16[jentry + ientry].entry_black_mate);
        }
  ...
  double compress_ratio_old =
      100.0 - compressed_size_old * 100.0 / uncompressed_size_old;
  ...
  PRINTF("nall_draw=%lld nall_i8=%lld nall_i16=%lld ratio_i8=%.2f\n",
         ...
         nall_i8,
         nall_i16,
         nall_i8 * 100.0 / npage_new);

  PRINTF("nall_inflated=%lld\n", nall_inflated);

  for (int ibucket = 0; ibucket < ARRAY_PAGE_SIZE; ibucket++)
  {
    if (nbucket[ibucket] > 0)
      PRINTF("ibucket=%d nbucket=%lld\n", ibucket, nbucket[ibucket]);
  }

  double compress_ratio_new =
      100.0 - compressed_size_new * 100.0 / uncompressed_size_old;
  ...
  for (ipage_new = 0; ipage_new < npage_new; ipage_new++)
  {
    ...
    i64_t seek_new =
        sizeof(i8_t) + sizeof(i64_t) + sizeof(i16_t) +
        ipage_new * (sizeof(i64_t) + sizeof(i16_t) + sizeof(ui32_t));
    ...
    HARDBUG(fread(source_new, sizeof(i8_t), sourceLen_new, fnew) !=
            sourceLen_new)
    ...
    else if (source_new[0] == ALL_I8)
    {
      ...
      destLen_new =
          ZSTD_decompress(dest_new, destLen_new, source_new + 1, sourceLen_new);
