...
#define MARK_ARRAY_READ_ONLY(P, S) \
  register_heap(#P, __FILE__, __FUNC__, __LINE__, P, S, FALSE, TRUE);

#define MY_MALLOC_VOID(P, N) \
  HARDBUG(((P) = my_malloc(#P, __FILE__, __FUNC__, __LINE__, N)) == NULL)

#define MY_MALLOC_BY_TYPE(P, T, N)                                          \
  HARDBUG(                                                                  \
      ((P) = (T *)                                                          \
           my_malloc(#P, __FILE__, __FUNC__, __LINE__, sizeof(T) * (N))) == \
      NULL)

#define MY_FREE_AND_NULL(P) \
  {                         \
    my_free(P);             \
    (P) = NULL;             \
  }
...
#define BSTRING(B)                                     \
  ...
  if (B == NULL)                                       \
  {                                                    \
    PUSH_NAME(#B) HARDBUG((B = bfromcstr("")) == NULL) \
  }
...
#define CSTRING(C, N)                                  \
  ...
  if (C == NULL)                                       \
  {                                                    \
    PUSH_NAME(#C) HARDBUG((C = malloc(N + 1)) == NULL) \
  }
...
ui64_t return_XXH3_64(const void *, size_t);

void mark_pointer_read_only(void *);
void register_heap(char *, char *, const char *, int, void *, size_t, int, int);
