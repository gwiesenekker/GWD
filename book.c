  ...
  int arg_depth, int arg_alpha, int arg_beta)
{
  ...
  if ((moves_list.ML_ncaptx == 0) and (moves_list.ML_nmoves > 1))
  {
    ...
    position_id = query_position_id(arg_db, bdata(bfen),
                                    options.book_position_frequency);
  ...
  for (int imove = 0; imove < moves_list.ML_nmoves; imove++)
  {
    ...
    if ((moves_list.ML_ncaptx > 0) or (moves_list.ML_nmoves == 1))
    {
      ...
      int minimax_score  =
         -help_walk_book_alpha_beta(arg_db, object, arg_depth + 1,
                                    -arg_beta, -best_score);
    
      undo_move(object, imove, &moves_list, FALSE);

      if (minimax_score == SCORE_MINUS_INFINITY) return(SCORE_PLUS_INFINITY);
    ...
    else
    {
      ...
      if (evaluation == SCORE_PLUS_INFINITY) continue;
      ...
      if (minimax_score != SCORE_MINUS_INFINITY) temp_score = minimax_score;
    }
  ...
  bstring arg_bpv)
{
  ...
  for (int imove = 0; imove < moves_list.ML_nmoves; imove++)
  {
    ...
    else
    {
      ...
      int minimax_score = -help_walk_book_alpha_beta(arg_db, object, arg_depth,
      ...
      HARDBUG(bformata(bpv, "%s %s[%d, %d]",
              ...
              evaluation, minimax_score) == BSTR_ERR)
  ...
  bstring arg_bbook_move)
{
  ...
  if ((moves_list.ML_ncaptx == 0) and (moves_list.ML_nmoves > 1))
  {
    ...
    position_id = query_position_id(book, bdata(bfen),
                                    options.book_position_frequency);
  ...
  for (int imove = 0; imove < moves_list.ML_nmoves; imove++)
  {
    ...
    if ((moves_list.ML_ncaptx > 0) or (moves_list.ML_nmoves == 1))
    {
      ...
      if (minimax_score == SCORE_MINUS_INFINITY) continue;
    ...
    else
    {
      ...
      int evaluation = query_evaluation(book, move_id);
      ...
      if (options.book_minimax)
      {
        ...
        int minimax_score = -help_walk_book_alpha_beta(book, object, 0,
                                                       SCORE_MINUS_INFINITY,
                                                       SCORE_PLUS_INFINITY);
    ...
    PRINTF("book_move=%s evaluation=%d minimax_score=%d temp_score=%d\n",
           bdata(bmove), evaluation, minimax_score, temp_score);
...
local void query_positions(sqlite3 *arg_db, int arg_depth)
{
  ...
  char *sql =
    "SELECT id, depth FROM positions "
  ...
  HARDBUG(sqlite3_bind_int(stmt, 1, arg_depth) != SQLITE_OK)
...
local void add_evaluations(my_sqlite3_t *arg_db, search_t *arg_search,
  i64_t arg_position_id, moves_list_t *arg_moves_list, int arg_centi_seconds)
{
...
local void add_evaluations2book_worker(my_sqlite3_t *arg_db,
  search_t *arg_search, int arg_centi_seconds)
{
...
void add_evaluations2book(my_sqlite3_t *arg_db, int arg_centi_seconds,
  int arg_add_moves)
{
...
local void add_position(my_sqlite3_t *arg_db, search_t *arg_search,
  int arg_depth)
{
  moves_list_t moves_list;
...
local void expand_book_worker(my_sqlite3_t *arg_db, i64_t arg_position_id,
  search_t *arg_search, int arg_depth, int arg_centi_seconds)
{
  moves_list_t moves_list;
  ...
  add_evaluations(arg_db, arg_search, arg_position_id, &moves_list,
                  arg_centi_seconds);
...
void expand_book(my_sqlite3_t *arg_db, char *arg_fen,
  int arg_depth, int arg_centi_seconds)
{
  search_t search;
  ...
  my_mpi_barrier_v3("after adding positions",
                    my_mpi_globals.MMG_comm_slaves,
                    SEMKEY_ADD_POSITIONS_BARRIER, FALSE);
  ...
  my_mpi_barrier_v3("after querying positions",
                    my_mpi_globals.MMG_comm_slaves,
                    SEMKEY_QUERY_POSITIONS_BARRIER, FALSE);
  ...
  for (i64_t iposition = 0; iposition < npositions; iposition++)
  {
    ...
    HARDBUG(query_position_fen(arg_db->MS_db, positions[iposition].P_id,
                               bfen) == INVALID)
    ...
    expand_book_worker(arg_db, positions[iposition].P_id, &search, arg_depth,
                       arg_centi_seconds);
  }
  ...
  my_mpi_allreduce(MPI_IN_PLACE, &npositions_mpi, 1, MPI_LONG_LONG_INT,
